...
function resultSearch() {
    ...
    if(sort == '제목')  sortSearch = 'title';
    else if(sort == '내용') sortSearch = 'content';
    else if(sort == '작성자')   sortSearch = 'id';

    let countQuery = `select count(*)as totalcount from tcs_homepage_db.board where ${sortSearch} like concat ('%','${search}','%')`
    ...
    for (i in queryData) {
        ...
        let yourQuery = `select * from tcs_homepage_db.board where ${sortSearch} like concat ('%','${search}','%') 
                         order by boardNum desc limit ${(cnt * countList) - countList},${countList}`
        new TCSPHP().phpData(`../assets/php/twQuery.php?do=get&query=${yourQuery};`, (getData) => {
            ...
            checkSession2((data) => {
                ...
                new TCSPHP().phpData(`../assets/php/twQuery.php?do=get&query=${yourQuery};`, (getData) => {
                    ...
                    if(selectData == ''){
                        str += `<tr>`
                        str += `<td colspan="5">검색된 게시물이 없습니다.</td>`
                        str += `</tr>`
                    }
                    try{
        ...
        if (icntDown >= 1 && startPage > 1) {
            stb += `<li class="page-item"><a class="page-link" href="boardSearch.html?icount=${startPage-1}&search=${search}&sort=${sort}">이전</a></li>`;
        }
        let classTwo = `page-link${cnt}` 
        for (var icount = startPage; icount<endPage ; icount++){
            stb += `<li class="page-item page-link" id="page-item"><a class="${classTwo}" id="page-link" href="boardSearch.html?icount=${icount}&search=${search}&sort=${sort}">${icount}</a></li>`                                      
        }
        stb += `<li class="page-item page-link"><a class="page-link${endPage}" href="boardSearch.html?icount=${endPage}&search=${search}&sort=${sort}">${endPage}</a></li>`
        var icntUp = parseInt(cnt);
        icntUp++
    
        if (icntUp <= totalPage && endPage < totalPage) {
            stb += `<li class="page-item"><a class="page-link" id="up_page" href="boardSearch.html?icount=${endPage+1}&search=${search}&sort=${sort}">다음</a></li>`
        }
...
$(document).on("click", "#boardList", function() {
    ...
    let yourQuery = `update tcs_homepage_db.board set views = views %2B 1 where boardNum = ${num}`
    new TCSPHP().phpData(`../assets/php/twQuery.php?do=get&query=${yourQuery};`, (getData) => {
        const updateViews = [];
